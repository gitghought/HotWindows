...
RegRead,Path_list,HKEY_CURRENT_USER,HotWindows,Path_list	;自定义程序添加规则
if not Styles
	Styles=0x860F0000,0x860E0000,0x36CF0000,0x17CF0000,0x84C80000,0xB4CF0000,0x94CA0000,0x95CF0000,0x94CF0000,0x94000000	;默认匹配的样式
if not Hot_key{
...
;<<<<<<<<<<<<声明全局变量>>>>>>>>>>>>
global Styles,Path_data,Show_mode,Key_History,WHERE_list,Path_list
...
TrayTip,HotWindows,正在准备请等待准备完成%additional%,,2
;<<<<<<<<<<<<加载列表>>>>>>>>>>>>
Load_list()	;创建初始程列表
...
loop{
	...
	WinGet,Get_Path,ProcessPath,ahk_id %Wina_id%
	WinGet,Get_Style,Style,ahk_id %Wina_id%
	WinGetTitle,Title,ahk_id %Wina_id%
	SplitPath,Get_Path,OutFileName,OutDir,OutExtension,OutNameNoExt,OutDrive
	if not Sql_Get("SELECT COUNT(*) FROM Quick WHERE Path='" Get_Path "'")
	{
		SQL_Run("DELETE FROM Quick WHERE Title='" OutNameNoExt "'")
		SQL_Run("Insert INTO Quick (Title,Path) VALUES ('" OutNameNoExt "','" Get_Path "')")
	}
	;IfNotInString,Styles,GetStyle
	if Get_Style
...
if GetKeyState(HotWindows,"P"){
	...
	SQL_List("SELECT * FROM (SELECT Now_list.Title,Now_list.Pid,Now_list.Path,Now_list.GetStyle,Activate.Times FROM Now_list LEFT JOIN Activate ON Now_list.Title=Activate.Title UNION SELECT Quick.Title,Quick.Pid,Quick.Path,Quick.GetStyle,Activate.Times FROM Quick LEFT JOIN Activate ON Quick.Title=Activate.Title) ORDER BY Times DESC,Pid DESC",Key_History)
...
Load_list(){
	...
	loop,Parse,Path_list,`n
	{
		loop,%A_LoopField%
		{
			...
			Path_Add:=A_LoopFileLongPath
			;if OutExtension=lnk
			;{
			;	FileGetShortcut,%A_LoopFileLongPath%,Path_Add
			;	SplitPath,Path_Add,OutFileName,OutDir,OutExtension,OutNameNoExt,OutDrive
			;}
			if not Sql_Get("SELECT COUNT(*) FROM Quick WHERE Path='" Path_Add "'")
			{
				SQL_Run("DELETE FROM Quick WHERE Title='" OutNameNoExt "'")
				SQL_Run("Insert INTO Quick (Title,Path) VALUES ('" OutNameNoExt "','" Path_Add "')")
			}
		}
	}
}
